...
def predict_total_and_return_history(user_id: str):
    """Generate total daily + weekly predictions for a user and return recent history."""
    try:
        today = datetime.now().date()
        now = datetime.now()
        current_week = f"{((now.day - 1) // 7) + 1:02d}"
        ...
        current = start_date
        while current <= today:
            result = total_daily_prediction(user_id)
            if result is not None:
                payload = {
                    "predicted_kWh": round(result, 2),
                    "timestamp": f"{current.strftime('%Y-%m-%d')} 00:05:00",
        ...
        if not last_week or (last_week[0], last_week[1], last_week[2]) < target_week:
            result = total_weekly_prediction(user_id)
            if result is not None:
                payload = {
                    "predicted_kWh": round(result, 2),
                    "timestamp": f"{now.strftime('%Y-%m-%d')} 00:05:00",
